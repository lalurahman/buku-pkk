...
use App\DataTables\Village\ActivityDataTable;
...
class ActivityController extends Controller
{
    ...
    {
        return $dataTable->render('pages.village.activity.index');
    }
    ...
    public function show($id)
    {
        ...
        if (!$activity) {
            return redirect()->route('village.activities.index')
                ->with('error', 'Data kegiatan tidak ditemukan');
        }

        // Get village_id dari user yang login
        $userHasVillage = UserHasVillage::where('user_id', $this->user()->id)->first();
        ...
        return view('pages.village.activity.show', [
            'activity' => $activity,
